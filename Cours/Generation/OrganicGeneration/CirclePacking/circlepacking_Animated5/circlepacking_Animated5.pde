ArrayList<Circle> circleList;
float overlappingMargin;
float minRadius;
float maxRadius;
float radius;
float angle;

void setup() {
  size(1280, 720, P2D);
  smooth(8);
  circleList = new ArrayList<Circle>();
  overlappingMargin = 2;
  minRadius = 150;
  maxRadius = 300;
  radius = minRadius;
  angle = 0;
}

void draw() {
  background(20);

  Circle newCircle = getNewCircle();
  if (newCircle != null) {
    circleList.add(newCircle);
  }

  for (int i=0; i<circleList.size(); i++) {
    Circle circle = circleList.get(i);
    if (circle.growing) {
      if (circle.edges() == true) {
        circle.growing = false;
      } else {
        for (int j=0; j<circleList.size(); j++) {
          Circle otherCircle = circleList.get(j);
          if (circle != otherCircle) {
            float d = dist(circle.x, circle.y, otherCircle.x, otherCircle.y);
            if (d - overlappingMargin < (circle.radius + otherCircle.radius)) {
              circle.growing = false;
              break;
            }
          }
        }
      }
    }
    circle.grow();
  }

  //draw line between two cell
  for (int i=0; i< circleList.size()-1; i++) {
    Circle circle = circleList.get(i);
    for (int j=0; j<circleList.size(); j++) {
      Circle nextCircle = circleList.get(j);
      float distance = dist(circle.x, circle.y, nextCircle.x, nextCircle.y);
      if (distance - overlappingMargin <= circle.radius + nextCircle.radius) {
        stroke(255);
        line(circle.x, circle.y, nextCircle.x, nextCircle.y);
      }
    }
  }
}

Circle getNewCircle() {
  radius = minRadius + sin(noise(frameCount * 0.1) * PI) * (maxRadius - minRadius);
  float theta = angle;
  angle += 0.01;

  float x_ = width/2 + cos(theta) * radius;
  float y_ = height/2 + sin(theta) * radius;

  boolean validCircle = true;
  for (int i=0; i<circleList.size(); i++) {
    Circle circle = circleList.get(i);
    float distance = dist(x_, y_, circle.x, circle.y);
    if (distance < circle.radius) {
      validCircle = false;
      break;
    }
  }
  if (validCircle == true) {
    return new Circle(x_, y_);
  } else {
    return null;
  }
}