...
function draw(){
	...
	for(var i=0; i<ballList.length; i++){
		...
		for(var j=0; j<ballList.length; j++){
			...
			if(other != ball){
				var distance = dist(ball.x, ball.y, other.x, other.y);
				var maxDist = (ball.radius + other.radius) * 4;
				if(distance < maxDist){
					var fillInc = 1.0 - norm(distance, 0, maxDist);
					stroke(255, 255 * fillInc);
					line(ball.x, ball.y, other.x, other.y);
				}
			}else{
